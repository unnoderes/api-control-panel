...
function formatContextWindow(value: EnrichedModelDto['context_window']): string {
  ...
  if (n >= 1_000_000) return `${Number((n / 1_000_000).toFixed(Number.isInteger(n / 1_000_000) ? 0 : 1))}M tokens`;
  if (n >= 1_000) return `${Number((n / 1_000).toFixed(Number.isInteger(n / 1_000) ? 0 : 1))}k tokens`;
  return `${n} tokens`;
}
...
function inferModelGroupLabel(item: EnrichedModelDto): string {
  ...
  if (key.startsWith('gpt-') || key.startsWith('o1') || key.startsWith('o3') || key.startsWith('o4')) return 'OpenAI';
  if (key.startsWith('claude-')) return 'Anthropic';
  if (key.startsWith('gemini-')) return 'Google';
  if (key.startsWith('qwen-')) return 'Alibaba Cloud';
...
function mapModelStatus(item: EnrichedModelDto): Pick<ModelSummary, 'status' | 'statusLabel'> {
  if (item.deprecated || item.status?.toLowerCase() === 'deprecated') return { status: 'deprecated', statusLabel: 'Deprecated' };
  if (item.enabled === false || item.status?.toLowerCase() === 'disabled') return { status: 'coming_soon', statusLabel: 'Unavailable' };
